...
export function sortColorsChromatic(colors) {
  ...
  return [...colors].map((color) => {
    ...
    // Es gris si saturación < 12%
    const isGray = hsl.s < 12;
    
    // Rotar hue para que amarillo (60°) sea 0
    let sortHue = (hsl.h - 60 + 360) % 360;
    
    // Banda de 15° cada una
    let hueBand = Math.floor(sortHue / 15);
    
    // Score único: banda primero, luego luminosidad invertida (claro primero)
    // Multiplicar banda por 1000 para que sea prioritario
    let sortScore = isGray ? 99000 + (100 - hsl.l) : (hueBand * 1000) + (100 - hsl.l);
