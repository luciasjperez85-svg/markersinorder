...
import { Palette, Sparkles, Upload, Grid3X3, Camera, AlertTriangle } from 'lucide-react';
import { sortColorsByHue, sortColorsBySaturation, sortColorsByLightness, sortColorsChromatic } from './utils/colorUtils';
import { Button } from './components/ui/button';

// Error Boundary para evitar que la app se cuelgue
          ...
          <Card className="max-w-md">
            <CardContent className="pt-6 text-center space-y-4">
              ...
              <p className="text-muted-foreground">
                No te preocupes, tus colores guardados están a salvo.
              </p>
...
const ColorSorterApp = () => {
  ...
  const [sortOrder, setSortOrder] = useState('hue');
  ...
  useEffect(() => {
    try {
      ...
      if (savedSortOrder && ['hue', 'saturation', 'lightness', 'chromatic'].includes(savedSortOrder)) {
        setSortOrder(savedSortOrder);
      }
    } catch (error) {
      console.error('Error loading saved colors:', error);
      // Si hay error, limpiar localStorage corrupto
      localStorage.removeItem('chromasort-colors');
    } finally {
      setIsLoading(false);
    }
  }, []);

  // Save colors to localStorage whenever they change (with error handling)
  useEffect(() => {
    if (!isLoading && Array.isArray(colors)) {
      try {
